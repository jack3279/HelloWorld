...
function bakeSlime(pose) {
  return bake({ skin, pose, canvas: swellCanvas(pose, SPRITE), tolerance: TOLERANCE, model });
}
...
await writeHeroSvg({
  ...
  desc: "Voxel slime facing right with the cube turned 45 degrees so both eyes and the mouth stay readable. A cube, no limbs.",
  groupId: "slime",
  skin,
  pose: idleA(),
  tolerance: TOLERANCE,
...
const walkSprites = catalog().map((entry) => {
  const baked = bakeSlime(entry.pose);
  return { ...entry, ...baked };
});
...
for (const [prefix, label, count, sample] of [
  ["hurt", "Hurt", HURT_FRAMES, sampleHurt],
  ["death", "Death", DEATH_FRAMES, sampleDeath],
]) {
  const frames = Array.from({ length: count }, (_, i) => {
    const baked = bakeSlime(sample(i / Math.max(1, count - 1)));
    return { id: `${prefix}-${i}`, label: `${label} ${i + 1}/${count}`, ...baked };
  });
  await writeFrames({
    generator: "scripts/generate-slime.mjs",
    groupId: "slime",
...
const walk = Array.from({ length: WALK_FRAMES }, (_, i) => {
  const baked = bakeSlime(walkFrame(i / WALK_FRAMES));
