...
describe("html game demo", () => {
  it("ships a standalone page that loads vanilla JS", async () => {
    ...
    assert.match(game, /buildQuarryWorld/);
    assert.match(game, /buildBridgeWorld/);
    assert.match(game, /buildNetherWorld/);
    assert.match(game, /function tryPlaceBlock/);
    assert.match(game, /function placeSupport/);
    assert.match(game, /function placeCells/);
    assert.match(game, /stats\.mineItems/);
    assert.match(game, /stats\.placeItems/);
    assert.match(html, /id="level-list"/);
    assert.match(html, /九个关卡/);
    assert.match(html, /通关后按/);
    assert.match(html, /采石/);
    assert.match(html, /搭桥/);
    assert.match(html, /放置/);
