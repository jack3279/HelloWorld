// Wolf: side-view SVG, walk / idle / rest sprites, and three Skottie loops.
//
// Usage: node scripts/generate-wolf.mjs [--skin=<png>]
import { dirname, resolve } from "node:path";
import { fileURLToPath } from "node:url";
import { loadWolfSkin, parseArgs } from "./lib/steve-model.mjs";
...
const model = WOLF_MODEL;
const args = parseArgs(process.argv.slice(2));
const skin = await loadWolfSkin(args.get("skin"));
if (skin.width < 64 || skin.height < 32) throw new Error("expected a 64×32 wolf texture");

const hero = resolve(__dirname, "../assets/wolf-side.svg");
await writeHeroSvg({
  generator: "scripts/generate-wolf.mjs",
  out: hero,
  title: "Wolf, side view",
  desc: "Voxel wolf facing right with the head turned 45 degrees so the face, ears, and muzzle stay readable. Mane, four thin legs, and a tail.",
  groupId: "wolf",
  skin,
  pose: idleA(),
  tolerance: TOLERANCE,
...
const sprites = await writeSpriteKit({
  ...
  sprite: SPRITE,
  frames: walkSprites,
  outDir: resolve(__dirname, "../assets/wolf-sprites"),
  stillPath: resolve(__dirname, "../assets/wolf-walk.svg"),
  stillLabel: "Wolf, walking, facing right",
});
...
await writeScene(
  resolve(ROOT, "public/projects/wolf/scene-1"),
  flipbook({
    name: "Wolf — Side idle",
    w: SPRITE.w,
    h: SPRITE.h,
...
await writeScene(
  resolve(ROOT, "public/projects/wolf/scene-2"),
  flipbook({
    name: "Wolf — Walk",
    w: SPRITE.w,
    h: SPRITE.h,
...
await writeScene(
  resolve(ROOT, "public/projects/wolf/scene-3"),
  flipbook({
    name: "Wolf — Rest",
    w: SPRITE.w,
    h: SPRITE.h,
